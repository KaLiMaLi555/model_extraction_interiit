...
def get_evaluation_points(N, C, L, S, m, dim, epsilon, pre_x, x, args):
...
def approximate_gradients_movinet(args, victim_model, clone_model, x, epsilon=1e-7, m=5, verb=False, num_classes=400, device="cpu", device_tf='/device:GPU:0', pre_x=False):
    ...
    with torch.no_grad():
        ...
        for i in (range(N * m // max_number_points + 1)):
            ...
            # Teacher expects: b, f, h, w, c = N, L, S, S, C
